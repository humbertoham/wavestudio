...
export const LEGACY_WELLHUB_MONTHLY_CREDITS = 15;
...
export function parseWellhubPlan(value: unknown): WellhubPlan | null {
  ...
  return isWellhubPlan(normalized) ? normalized : null;
}
...
export function getCorporateGrantConfig(
  ...
  options: { allowLegacyWellhubFallback?: boolean } = {}
): CorporateGrantConfig | null {
  if (affiliation === Affiliation.WELLHUB) {
    const planCredits = getWellhubPlanCredits(wellhubPlan);
    if (planCredits && wellhubPlan) {
      return {
        classesGranted: planCredits,
        packId: WELLHUB_PLAN_PACK_IDS[wellhubPlan],
      };
    }

    if (options.allowLegacyWellhubFallback) {
      return {
        classesGranted: LEGACY_WELLHUB_MONTHLY_CREDITS,
        packId: LEGACY_WELLHUB_PACK_ID,
        requiresPlan: true,
      };
    }
...
export async function ensureCorporatePacks(client: PackClient) {
  await client.pack.upsert({
    ...
    update: {
      ...
      classes: LEGACY_WELLHUB_MONTHLY_CREDITS,
      price: 0,
      validityDays: 31,
      isActive: false,
      isVisible: false,
      oncePerUser: false,
      classesLabel: `${LEGACY_WELLHUB_MONTHLY_CREDITS} clases`,
    },
    create: {
