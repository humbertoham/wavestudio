...
describe("PATCH /api/admin/users/[id]/role", () => {
  ...
  it.each(["COACH", "USER", "ADMIN"] as const)(
    "lets admins assign %s",
    async (role) => {
      ...
      const res = await PATCH(req(role), ctx());
  ...
  it("returns a clear migration error when COACH is missing in the database enum", async () => {
    ...
    await expect(res.json()).resolves.toMatchObject({
      ...
      message:
        "La migracion de roles no esta aplicada en la base de datos. Ejecuta las migraciones antes de asignar Coach.",
    });
